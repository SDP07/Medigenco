<div class="modal-header">
    <h5 class="modal-title strong-600 heading-5" hidden>{{ translate('Order id')}}: HGR{{ mt_rand(100,999) }}{{ date('dY') }}</h5>
                    ...
                    <table class="details-table table table-responsive">
                        <thead>
                            <tr>
                                ...
                                <th hidden>{{ translate('Variation')}}</th>
                                <th hidden>{{ translate('Quantity')}}</th>
                                <th hidden>{{ translate('Delivery Type')}}</th>
                                <th hidden>{{ translate('Price')}}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null)
                                            <a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank">{{ $orderDetail->product->name }}</a><br>
                                            {{ single_price($orderDetail->price) }} X {{ $orderDetail->quantity }}<br>
                                    ...
                                    <td hidden>{{ single_price($orderDetail->price) }}</td>
                                    @if ($refund_request_addon != null && $refund_request_addon->activated == 1)
                                        @php
