...
<div class="row">
	<div class="col-lg-8 col-lg-offset-2">
		<form class="form form-horizontal mar-top" action="{{route('clinic.doctor.update',$doctorDeta->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
			@csrf
						...
						<div class="col-lg-7">
							<select class="form-control demo-select2-placeholder" name="doctor_id" id="category_id" required>
								@foreach($doctors as $doctor)
									<option value="{{$doctor->id}}" <?php if($doctor->id == $doctorDeta->doctor_id){ echo 'selected';}  ?>>{{__($doctor->Name)}}</option>
								@endforeach
							</select>
						</div>
						...
						<div class="col-lg-7">
							<select class="form-control demo-select2-placeholder" name="clinic_id" id="category_id" required>
								@foreach($clinics as $clinic)
									<option value="{{$clinic->id}}"  <?php if($clinic->id == $doctorDeta->clinic_id){ echo 'selected';}  ?>>{{__($clinic->clinic_name)}}</option>
								@endforeach
							</select>
						</div>
			...
			<div class="panel" id="timingDet">
				...
				<div class="panel-body">
					<div class="form-group">
						...
						<div class="col-lg-9">
							<input type="text" class="form-control" name="day" placeholder="{{ translate('Ex:Sunday') }}" value="{{ $doctorDeta->day }}">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-2 control-label">{{translate('Start Time')}}</label>
						<div class="col-lg-9">
							<input type="time" class="form-control" name="start" placeholder="{{ translate('Starting Time') }}" value="{{ $doctorDeta->start }}">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-2 control-label">{{translate('End Time')}}</label>
						<div class="col-lg-9">
							<input type="time" class="form-control" name="end" placeholder="{{ translate('Ending Time') }}" value="{{ $doctorDeta->end }}">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-9">
							<input type="number" class="form-control" name="price"  placeholder="{{ translate('Price') }}" value="{{ $doctorDeta->fees }}">
						</div>
					</div>
					<div class="form-group">
						<label class="col-lg-2 control-label">{{translate('Discount')}}</label>
						<div class="col-lg-9">
							<input type="number" class="form-control" name="discount"  placeholder="{{ translate('Discount') }}" value="{{ $doctorDeta->discount }}">
						</div>
					</div>
